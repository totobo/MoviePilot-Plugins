...
### SubscribeEpisodeGuard — 订阅集数守卫（自研，v1.1.0）
...
- 处理记录详情页：插件管理 → 本插件 → 详情，展示运行状态、今日拦截/观察待确认/巡检缺口统计，
  与否决/观察/放行/缓兵/巡检/复活的逐条判定记录（保留 90 天 / 500 条）；另有 `GET /api/v1/plugin/subscribeepisodeguard/logs` 供外部拉取；
- **安装后默认停用**：进入插件配置勾选启用、选好模式并保存后才生效；首次启用建议保持**观察模式**（只记日志不干预），确认判定无误后切生效模式；
- 保守判定：仅电视剧、非手动集数、宽限期内（从 max(订阅创建, 剧集首播) 起算，提前订阅未开播剧不失效）、TMDB 未完结、最新集在播窗口内才拦截；
- 要求 MoviePilot >= v2.15.0；回滚 = 界面禁用插件，零数据改动。

### SubscribeGroupFix — 订阅规则自动填充·修复版（二开 thsrite/SubscribeGroup v2.8.7）
上游功能（下载后固化订阅条件 / 按二级分类预填规则），修复三处：
...
3. **识别词还原（fix.3）**：不再简单丢弃被替换信息——用 apply_words/词表反查 + 锚点差分
   把 `60帧` 还原为原始标题里的 `60FPS`，保住精度；规则形式升级为同现断言
   `(?=.*60FPS)(?=.*HHWEB)`（不限词序，组名前置排版也能命中）。还原不了仍走降级链路，
   自匹配闸门始终兜底：宁缺毋滥，绝不写死规则。
