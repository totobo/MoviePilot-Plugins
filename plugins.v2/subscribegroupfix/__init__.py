...
class SubscribeGroupFix(_PluginBase):
    ...
    # 插件配置项ID前缀
    plugin_config_prefix = "subscribegroupfix_"
    # 加载顺序
    plugin_order = 26
    # 可使用的用户级别
    auth_level = 2
    ...
    def download_notice(self, event: Event = None):
        ...
        if event:
            ...
            # [FIX subscribegroupfix] 去重键加入季号：上游 type:tmdbid 会让多季剧第二季不再固化
            handle_key = f"{download_history.type}:{download_history.tmdbid}:{download_history.seasons}"
                    ...
                    if candidate:
                        ...
                        try:
                            if raw_title and re.search(candidate, raw_title, re.IGNORECASE):
                                ...
                                logger.warn(
                                    f"订阅记录:{subscribe.name} 固化规则 {candidate} 无法匹配原始种子名"
                                    f"（识别词替换错位），降级为制作组: {resource_team}")
                                update_dict['include'] = resource_team
                            else:
                                logger.warn(
                                    f"订阅记录:{subscribe.name} 固化规则 {candidate} 无法匹配原始种子名，放弃填充")
                        except re.error:
                            if resource_team:
                                logger.warn(f"订阅记录:{subscribe.name} 候选规则非法正则，降级为制作组: {resource_team}")
                                update_dict['include'] = resource_team
                # 站点
